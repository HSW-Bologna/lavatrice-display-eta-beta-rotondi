#ifndef AUTOGEN_FILE_STRINGS_H_INCLUDED
#define AUTOGEN_FILE_STRINGS_H_INCLUDED

typedef enum {
    STRINGS_DIAGNOSI = 0,
    STRINGS_INGRESSI,
    STRINGS_USCITE,
    STRINGS_LIVELLO,
    STRINGS_CALIBRAZIONE,
    STRINGS_AZZERA_LITRI,
    STRINGS_AZZERA,
    STRINGS_CALDA,
    STRINGS_FREDDA,
    STRINGS_AMMOLLO,
    STRINGS_PRELAVAGGIO,
    STRINGS_LAVAGGIO,
    STRINGS_RISCIACQUO,
    STRINGS_CENTRIFUGA,
    STRINGS_SROTOLAMENTO,
    STRINGS_ATTESA_OPERATORE,
    STRINGS_SCARICO,
    STRINGS_TEMPERATURA,
    STRINGS_GETTONI,
    STRINGS_LINEA,
    STRINGS_TOTALE,
    STRINGS_CASSA,
    STRINGS_LED,
    STRINGS_SPENTO,
    STRINGS_ROSSO,
    STRINGS_VERDE,
    STRINGS_BLU,
    STRINGS_GIALLO,
    STRINGS_VIOLETTO,
    STRINGS_TURCHESE,
    STRINGS_BIANCO,
    STRINGS_SERRATURA,
    STRINGS_APERTO,
    STRINGS_CHIUSO,
    STRINGS_APRI,
    STRINGS_CHIUDI,
    STRINGS_CHIAVISTELLO,
    STRINGS_DIGITALE,
    STRINGS_MECCANICA,
    STRINGS_MOTORE_AVANTI,
    STRINGS_MOTORE_INDIETRO,
    STRINGS_OBLO,
    STRINGS_ARCHIVIAZIONE,
    STRINGS_IMPOSTAZIONI,
    STRINGS_CHIAVETTA,
    STRINGS_ERRORE_DI_COMUNICAZIONE,
    STRINGS_RIPROVA,
    STRINGS_IMPORTA_UNA_CONFIGURAZIONE,
    STRINGS_ESPORTA_LA_CONFIGURAZIONE,
    STRINGS_NESSUNA_CONFIGURAZIONE_TROVATA,
    STRINGS_OPERAZIONE_FALLITA,
    STRINGS_OPERAZIONE_CONCLUSA_CON_SUCCESSO,
    STRINGS_SCELTA_PROGRAMMA,
    STRINGS_ERRORE_EEPROM,
    STRINGS_ALLARME_SPEGNIMENTO,
    STRINGS_ALLARME_COMUNICAZIONE,
    STRINGS_ALLARME_OBLO_APERTO,
    STRINGS_ALLARME_OBLO_SBLOCCATO,
    STRINGS_ALLARME_EMERGENZA,
    STRINGS_ALLARME_SBILANCIAMENTO,
    STRINGS_ALLARME_CHIAVISTELLO_BLOCCATO,
    STRINGS_ALLARME_SERRATURA,
    STRINGS_ALLARME_LIVELLO,
    STRINGS_ALLARME_SERRATURA_FORZATA,
    STRINGS_ALLARME_TEMPO_SCADUTO_T1,
    STRINGS_ALLARME_INVERTER,
    STRINGS_ALLARME_FILTRO_APERTO,
    STRINGS_ALLARME_BLOCCO_BRUCIATORE,
    STRINGS_ALLARME_TEMPERATURA_1,
    STRINGS_ALLARME_APERTO_H2O,
    STRINGS_ALLARME_ACCELEROMETRO,
    STRINGS_ALLARME_ACCELEROMETRO_FUORI_SCALA,
    STRINGS_ALLARME_VELOCITA,
    STRINGS_ALLARME_NO_MOTO,
    STRINGS_ALLARME_NO_FERMO,
    STRINGS_ALLARME_TEMPERATURA,
    STRINGS_ALLARME_H2O_IN_VASCA,
    STRINGS_ALLARME_RIEMPIMENTO,
    STRINGS_ALLARME_RISCALDAMENTO,
    STRINGS_ALLARME_SCARICO,
    STRINGS_PAGARE_CASSA,
    STRINGS_PAGARE_GETTONE,
    STRINGS_PAGARE_MONETA,
    STRINGS_MACCHINA_ACCESA,
    STRINGS_FINE_LAVAGGIO,
    STRINGS_INIZIO_LAVAGGIO,
    STRINGS_LAVAGGIO_INTERROTTO,
    STRINGS_APRIRE_OBLO,
    STRINGS_STANDBY_SAPONI,
    STRINGS_ALLARME,
    STRINGS_PARAMETRI,
    STRINGS_PAUSA_LAVAGGIO,
    STRINGS_SCARICO_NECESSARIO,
    STRINGS_PRECARICA_IN_CORSO,
    STRINGS_ATTESA_LIVELLO_E_TEMPERATURA,
    STRINGS_ATTESA_LIVELLO,
    STRINGS_ATTESA_TEMPERATURA,
    STRINGS_RIEMPIMENTO,
    STRINGS_ATTESA_TERMODEGRADAZIONE,
    STRINGS_ATTESA_LIVELLO_SCARICO,
    STRINGS_PRESCARICO,
    STRINGS_PREPARAZIONE,
    STRINGS_RAGGIUNGIMENTO_VELOCITA,
    STRINGS_IN_FRENATA_CAUSA_SBILANCIAMENTO,
    STRINGS_ATTESA_FRENATA,
    STRINGS_SCARICO_FORZATO,
    STRINGS_RECUPERO,
    STRINGS_USCITA_LAVAGGIO,
    STRINGS_PROGRAMMI,
    STRINGS_CODICE,
    STRINGS_IN_MARCIA,
    STRINGS_ABILITA,
    STRINGS_DISABILITA,
    STRINGS_INFORMAZIONI,
    STRINGS_CANCELLARE_IL_PROGRAMMA,
    STRINGS_CANCELLARE_IL_PASSO,
    STRINGS_AVANZATE,
    STRINGS_COMUNICAZIONE,
    STRINGS_RIPRISTINO_A_IMPOSTAZIONI_DI_FABBRICA,
    STRINGS_ACCESSO,
    STRINGS_MOLTO_SPORCHI_CON_PRELAVAGGIO,
    STRINGS_MEDIO_SPORCHI_CON_PRELAVAGGIO,
    STRINGS_MOLTO_SPORCHI,
    STRINGS_MEDIO_SPORCHI,
    STRINGS_COLORATI,
    STRINGS_SINTETICI,
    STRINGS_PIUMONI,
    STRINGS_DELICATI_A_FREDDO,
    STRINGS_LANA,
    STRINGS_LINO_E_TENDAGGI,
    STRINGS_SOLO_CENTRIFUGA_1000_GIRI,
    STRINGS_SOLO_CENTRIFUGA_600_GIRI,
    STRINGS_SANIFICAZIONE,
    STRINGS_PRELAVAGGIO_CON_CENTRIFUGA,
    STRINGS_RISCIACQUO_CON_CENTRIFUGA,
    STRINGS_NUOVO_PASSO,
    STRINGS_DELICATO,
    STRINGS_ENERGICO,
    STRINGS_ACCELEROMETRO,
    STRINGS_AGGIORNA,
    STRINGS_AGGIORNAMENTO_FIRMWARE_CONCLUSO_RESETTARE_IL_DISPOSITIVO,
    STRINGS_AGGIORNAMENTO_FIRMWARE_IN_CORSO,
    STRINGS_AGGIORNAMENTO_FIRMWARE_FALLITO,
    STRINGS_PREZZO,
    STRINGS_LAVAGGIO_CONCLUSO_APRIRE_OBLO,
    STRINGS_ORA_DATA,
    STRINGS_LAVAGGIO_PROGRAMMATO,
    STRINGS_ORE,
    STRINGS_MINUTI,
    STRINGS_GIORNO,
    STRINGS_MESE,
    STRINGS_ANNO,
    STRINGS_RINOMINA_MACCHINA,
    STRINGS_VELOCITA,
    STRINGS_DURATA,
    STRINGS_PASSWORD,
    STRINGS_SAPONE,
    STRINGS_STATISTICHE,
    STRINGS_EVENTI,
    STRINGS_TOTALI,
    STRINGS_INTERROTTI,
    STRINGS_CONTINUI,
    STRINGS_ACCESO,
    STRINGS_LAVORO,
    STRINGS_MOTO,
    STRINGS_RISCALDAMENTO,
    STRINGS_ACQUA_FREDDA,
    STRINGS_ACQUA_CALDA,
    STRINGS_ACQUA_RECUPERO,
    STRINGS_ACQUA_FLUSSAGGIO,
    STRINGS_CHIUSURE,
    STRINGS_APERTURE,
    STRINGS_CICLI,
    STRINGS_TEMPI,
} strings_t;

extern const char *strings[174][2];

#endif
